...
const Post = {
  pl: async function (PLDTO, res, next) {
    const { PLName, PL100, PL300, PL500, userObj, checkDate, checkList } =
      PLDTO;
    console.log(PLDTO);
    console.log(checkList);
    let journalObj = await Journal.findAndCountAll({
      where: {
        [Op.or]: [
          { MemberId: userObj.id },
          {
...
const Get = {
  pl: async function (PLDTO, res, next) {
    ...
    resultTemp = {
      code: 200,
      result,
      currentHour: tempObj.currentHour,
      maxHour: tempObj.maxHour,
    };
    return resultTemp;
  },
